...
- Combining across media types (transcribe voice into the text run; interleaved media+text bursts) — v1 boundary rule handles pure-text; this is the v2 refinement
  - **Accepted v1 scope, recorded post-review (code review WR-02, 2026-07-27).** The v1 limitation on line 25 is broader than "drops an earlier text fragment" — the dedupe aborts every non-newest fragment while the combine merges only text, so a MIXED-type burst answers the newest fragment and permanently drops the rest (dropped content reaches neither the AI Agent nor Chat Memory, so it is absent from future context too). Three interleavings, all accepted for v1: `voice t0 + text t1` → the voice is never transcribed or answered; `text t0 + voice t1` → the text question is never answered; `text t0 + image t1` → the customer gets «отправьте текстом» while their actual text question is ignored (worst UX of the three). NOT a regression — do not file it as one at the 10-04 owner pass. The contained v2 improvement is to carry the window's pending text alongside the media winner (e.g. emit `pendingText` next to `combinedText` and concatenate after `Transcribe Audio`), which is a behavioral redesign needing its own live e2e; deliberately not attempted in the review-fix pass. Add the voice→text and text→image interleavings to the matrix when the v2 media work is planned.
- Trimming the existing humanizer `Pause Before Reading` now that the debounce already waited (future latency tune)
- **Suppression flag is read PRE-wait — accepted, recorded post-review (code review IN-04, 2026-07-27). Do not "fix" this ordering.** `Read Reply Mode` → `Suppressed?` runs *before* `Debounce Wait`, so an owner who flips a chat to «Вместе» during a fragment's ~8s window still gets that one fragment auto-replied (a one-message-deep race; the review itself calls it "likely acceptable"). Gate-BEFORE-debounce is a LOCKED design property, not an oversight: it is the mitigation for threat **T-10-01-03** (a debounce spliced before the gate would be an outright suppression *bypass*), it is asserted structurally by `verify-message-batching.py` (`Suppressed?` `main[0] == []` dead-end, `main[1]` → `Debounce Wait`), and it was verified behaviorally at UAT scenario 5 / T-10-04-02 (a suppressed execution dead-ends at `Suppressed?` and never reaches `Debounce Wait`). Moving the gate read after `Is Latest?` would invalidate that verified evidence and the closed security audit, so it is deliberately not a review-fix change. If it is ever revisited, the only safe shape is an ADDITIONAL re-read on the winner branch (one cheap Postgres read, only the winner pays it) — never a moved gate.
- **Telegram-only: aborted fragments are never marked read — deferred to the prod pass (code review IN-05, 2026-07-27).** Post-splice only the winning fragment reaches `Mark Read`. WhatsApp's `mark_all=true` sweeps the whole burst, but tapi's `message/mark/read` takes a single `message_id`, so the earlier fragments of a Telegram burst stay unread. Cosmetic only — read receipts, a slight dent in the humanizer illusion; no content is lost (the combine still answers the whole text run). Deliberately NOT fixed in the review-fix pass because **no burst-covering tapi form is confirmed available** and guessing at an API shape is off-limits: `ChatManager.cs` (the Mark Read builder, ~:2348) records that tapi's `mark/read` documents no `mark_all` param and that its bulk lever is `mark_all` on `messages/get` instead; that lever is still open question **#13** in `Tools/tapi/SHAPES.md`, deliberately never probed because it MUTATES the owner's real read state (`Tools/tapi/samples/INDEX.json`: *"13: mark_all read mutation — mutating, not run here"*); and hanging it off `Fetch Recent` is forbidden by two existing verifiers — batching Pitfall 5 (`verify-message-batching.py`: no `mark_all` on `Fetch Recent`) and `verify-telegram-parity.py`'s "Mark Read still has mark_all" assert — since marking read during the wait would defeat the deliberate downstream humanizer `Mark Read`. Any fix therefore needs a live tapi probe of SHAPES #13 first; schedule that with the prod pass.
