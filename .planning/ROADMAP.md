...
- [ ] 08-11-PLAN.md — [gap round 2] D2 (medium): Telegram reaction-identity fix — shared canonical-emoji normalizer (tapi base ❤ vs app ❤️) at every equality/dedup seam + persisted owner TG user-id + same-emoji fold (own reaction = ONE glyph/count 1, change = ONE pill, add-heart = ONE heart, remove stays removed); WhatsApp byte-identical; diagnosis-first (root cause A emoji-form + B identity-timing CONFIRMED from Tools/tapi/samples); Editor capture logs the echo hex for 08-16 (wave 5)
