...
- [ ] 08-12-PLAN.md — [gap round 2] D9 (low): Telegram sync-pill visibility — Editor diagnosis (H1 fast-sync / H2 event-vs-OnEnable / H3 channel-read timing / H4 occlusion) + pure minimum-visible-duration gate + switch-to-Telegram-while-syncing show; code-only unless occlusion is confirmed (then one scene-stamp checkpoint); WhatsApp byte-identical (wave 5)
