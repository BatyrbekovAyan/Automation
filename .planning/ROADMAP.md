...
**Success Criteria** (what must be TRUE):
  ...
  3. The n8n flow returns a versioned `{ text, label }[]` payload ranked best-first plus a correlation id, and the live provider gates behind WaitForChatFetchesToDrain, supersedes via correlation, and aborts on bot switch — never crossing or rendering a stale set.
