...
## Unity Adaptation (this project)

This skill provides the design judgment only. When implementing in this Unity app:
- Ignore the React/Tailwind implementation notes above — all building goes through `.claude/skills/unity-ui-builder/SKILL.md`, which is the authority on sizing, fonts, and rendering.
- All px/pt values in this skill are dp — convert to 1080×1920 canvas reference units per unity-ui-builder before using them.
- HTML is for throwaway mockups only (`/gsd-sketch` variants); the deliverable is always Unity canvas UI.
