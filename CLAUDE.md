...
- **Endpoints**:
  ...
  - `chat/delete` — **soft**-delete a chat (POST, body `{ recipient }` where recipient = chatId minus `@c.us`; success `status:"done"`). Wappi keeps the chat in `chats/filter` with a sticky `isDeleted:true`, but its `last_message_id`/`last_timestamp` still advance on new activity. Backs swipe-to-delete on the chat list: `ChatManager.DeleteChat.cs` removes the row optimistically (rollback on failure) + evicts `ChatHistoryCache`, and a **persistent per-bot watermark** (`deleted_chats.json` via `DeletedChatStore`, keyed chatId→last-message-unix-at-deletion) drives visibility. In `ParseChatsJson`, `DeletedChatRule.ShouldHide` hides a chat while its `last_timestamp <= watermark`, **revives** it when newer activity arrives, and **adopts** (hides) an externally-`isDeleted` chat it has no watermark for. Per-bot file ⇒ bots are independent; survives restart/bot-switch
