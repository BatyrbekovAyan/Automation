...
- **Endpoints**:
  ...
  - `chat/delete` — permanently delete a whole chat (POST, body `{ recipient }` where recipient = chatId minus `@c.us`; success `status:"done"`). Backs swipe-to-delete on the chat list: `ChatManager.DeleteChat.cs` removes the row optimistically (rollback on failure), evicts `ChatHistoryCache` + rewrites `chats.json`, and a session-scoped `DeletedChatGuard` (checked in `ParseChatsJson`) stops a deleted chat reappearing on the next sync
