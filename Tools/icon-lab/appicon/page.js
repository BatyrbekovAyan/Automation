...
const DIRS = [
  {
    key: 'S', name: 'Точка отсчёта',
    bet: 'U9 из прошлого раунда — тонированные чернила, соседние прорезаны до тёмного, выбранная залита цветом. От неё отличается всё, что ниже: выбранная теперь обведена белым, а тонировка стала градиентной, с глубиной.',
  },
  {
    key: 'A', name: 'Что внутри контура',
    bet: 'Контур один и тот же — белый, 30 единиц, 3% иконки. Меняется заливка внутри: тёмно-синий, чернила, градиент, почти чёрный, золото, серебро.',
  },
  {
    key: 'B', name: 'Как нарисован контур',
    bet: 'Внутри везде тёмно-синий, меняется сама линия: пустая, тонкая, со свечением, со стеклянным бликом.',
  },
  {
    key: 'C', name: 'Окружение',
    bet: 'Контур на месте — меняется то, что вокруг: убран тёмный колодец, или соседние тоже обведены вместо прорезки.',
  },
];

const VERDICT = {
  U9:  ['Заметность S8 на тёмном фоне', 'Плоская тонировка — без глубины'],
  V1:  ['Ровно по брифу: белый контур, тёмно-синий внутри', 'Интерьер и колодец близки по тону'],
  V2:  ['Самый яркий, ближе всех к U9', 'Цвет и контур спорят за внимание'],
  V3:  ['Единственный с живым цветом внутри', 'Градиент теряется на 54px'],
  V4:  ['Контур — единственный светлый элемент', 'Почти чёрный интерьер сливается с колодцем'],
  V5:  ['Синий и золото — классика «дорогого»', 'Тёплый цвет вне фирменной палитры'],
  V6:  ['Объём одной светлой полосы', 'На мелком неотличим от белой полосы'],
  V7:  ['Самый воздушный', 'Пустой контур читается как «не выбрано»'],
  V8:  ['Самый ювелирный на 1024', 'На 54px линия почти исчезает'],
  V9:  ['Свет и на кромке, и вокруг', 'Ореол размывает точность контура'],
  V10: ['Полоса становится стеклянной пластиной', 'Блик — деталь, невидимая на мелком'],
  V11: ['Плотнее и проще без колодца', 'Контур на тонировке контрастирует слабее'],
  V12: ['Самый графический', 'Три контура — линий больше, чем форм'],
};
...
<div class="wrap">
  <header class="top">
    ...
    <h1>Белый контур на чернилах</h1>
    <p>Восьмой заход, по вашему брифу: похожие на U9, но выбранная полоса <b>обведена белым</b>, внутри — тёмно-синий и другие цвета, и чтобы выглядело <b>премиально</b>. Премиальность заложена в семейство целиком: плоская тонировка U9 стала градиентной с глубиной, контур — точный, 3% иконки. Страница по умолчанию открыта в палитре «Чернильный»; двенадцать вариантов ниже отличаются тем, что внутри контура, как он нарисован и что вокруг.</p>
    <div class="facts">
      <span class="fact">Белый контур</span>
      <span class="fact"><b>12</b> вариантов</span>
      <span class="fact">Тонировка с глубиной</span>
...
<div class="wrap">
  ...
  <div class="next">
    ...
    <div class="warn"><b>Что изменила панель, а что нет.</b> Принята одна правка на всё семейство: тонировка фона поднята с 0.32/0.10 до 0.36/0.20 — скептик измерил, что при 0.10 правый-нижний угол проваливался ниже пола U9 и на тёмных обоях плитка «растворялась»; диагональ глубины при этом сохранена. Отбиты три: перестройка каркаса (диагноз опирался на неверную геометрию — перемычка 30 единиц, а не 16), «кольцо 30 везде» (семейство и так на 30, тонкий и пустой контур — намеренные пробы) и осветление тёмно-синего интерьера (см. V1). И одно про Android: под круглую маску 66% нынешняя марка не влезает — при экспорте передний слой adaptive-иконки будет уменьшен до 63% (проверено: 0.65 уже задевает торцы вырезов), градиент фона растянут по видимому окну маски, а иконка для карточки Google Play — квадрат, берётся мастер как есть. На этой странице показана маска iOS.</div>
  </div>
</div>
