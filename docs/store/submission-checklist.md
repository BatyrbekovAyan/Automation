...
- [ ] **Сборка и TestFlight-прогон (до сабмита):** шаги 1–2 ЗАКРЫТЫ 2026-09-06 — архив
  ...
  2026-09-06 ночь: в репо уже поднят номер сборки 2 → 3 (`buildNumber.iPhone: 3`) вместе со
  строкой резерва на «Подписке» — следующая загрузка в ASC = билд 3; повторный Bump перед ней
  не обязателен (даст 4 — тоже допустимо).
...
- [x] **Хвост, найденный 2026-09-06 при sandbox-проверке — ЗАКРЫТ тем же вечером (в репо;
  попадает в сборку с номером 3+):** докупка «500 диалогов» зачисляется (RevenueCat → n8n
  `RevenueCat Events` → `subscribers.topup_balance`, GetUsage отдаёт `topupBalance`), но на
  странице «Подписка» резерв был НЕ ВИДЕН, пока квота не исчерпана — счётчик печатает только
  «N из квоты» (резерв = резерв, решение 2026-08-26), а подсказка с резервом живёт лишь на
  вкладке «Боты» в состоянии Reserve/Warn. Теперь под полосой «Диалоги ИИ» стоит строка
  «Резерв: 2 000 диалогов — спишутся после квоты» (seam `SubscriptionPageRows.ReserveLine`,
  скрыта при нуле и до первого GetUsage; сцена — аддитивный
  `Tools/Billing/Add Subscription Reserve Line`). Знаменатель счётчика НЕ менялся. Открытый
  вопрос владельцу: показывать ли резерв и на вкладке «Боты» в состоянии Ok (сейчас подсказка
  там пустая, `BotsPageRows.MeterHint` → null).
