...
## Ревью №1 — отказ 2.1(b) и его причина (2026-09-10 → 2026-09-13)

- Подача 2026-09-06 21:05, билд 1.0 (2). 07.09 — автозапрос подтвердить цены «Сети»
  ($69.99/$699.99 = наша ₸-сетка), ответ отправлен, принят 10.09 22:42.
- 10.09 22:48 — Guideline 2.1(b): «the app failed to complete the in-app purchase»
  (iPad Air 11 M3 + iPhone 17 Pro Max, iOS 26.6.2). Вложений нет.
- РАЗБОР по серверу (n8n executions GetUsage/RevenueCat Events за 17:41–17:48 UTC): ревьюер
  трижды открывал Подписку/Боты, RevenueCat на его устройствах поднялся (RC-id в запросах),
  но ни одна покупка НЕ дошла до StoreKit/RevenueCat — ноль событий. Sandbox-аккаунт ревьюера
  подписок не имеет ⇒ пейволл открывался в состоянии «триал не начат», где главная кнопка
  читалась «Попробовать 5 дней бесплатно» и ЗАКРЫВАЛА пейволл, а покупка была второй,
  менее заметной кнопкой. Ревьюер жал большую кнопку — окна Apple не было.
- Отброшенные гипотезы (все проверены): страны/сторфронт (US-тестер получает продукты и
  покупает), Paid Apps Agreement (Active), сбой SDK на свежей установке (RC-id через 5 с,
  покупки E2E 13.09), «покупка прошла в фоне, пейволл не отреагировал» (событий нет).
- ФИКС (код, 2026-09-13): главная кнопка = «Оформить <тариф> — <цена>» ВО ВСЕХ состояниях,
  триал = вторая строка, закрывающая пейволл; мелкий шрифт всегда = автопродление
  (`PaywallRows.CtaText/TrialRow/FinePrintText`, страж `PaywallCtaWiringTests`).
  Заметки для ревью дополнены точным путём до покупки (п. 4).
- РЕСАБМИТ СДЕЛАН 2026-09-14 (билд 1.0 (3)): ответ в тред + видео чистого прогона,
  затем билд 3 на странице версии → Update Review → Resubmit to App Review.
  Прогон в состоянии ревьюера (TestFlight билд 3, свежая установка, НОВЫЙ US sandbox-тестер
  без истории покупок, настоящий Apple ID выведен из «Контент и покупки»): при запуске
  «Пробный · осталось 5 дн.», пейволл — CTA «Оформить Старт», под ней «Попробовать 5 дней
  бесплатно», INITIAL_PURCHASE Старт → апгрейд Бизнес → топ-ап 500 — всё E2E на сервере.
  Видео: `~/Desktop/ChooseReply-1.0-3-sandbox-purchase-2026-09-14.mp4` (H.264, домашний
  экран обрезан). Ловушки этого прогона: sandbox-тестером НЕЛЬЗЯ войти в TestFlight
  («iTunes account creation not allowed») — он живёт только в Настройки → Разработчик;
  TestFlight берёт его лишь при ВЫХОДЕ из «Контент и покупки»; на телефоне с историей
  покупок RevenueCat склеивает новый анонимный id со старыми (aliases) и показывает чужой
  активный тариф — чистое состояние даёт только тестер без истории.
- Побочные наблюдения того же прогона (не блокеры): после переустановки под Apple ID с
  активной подпиской сервер до первого события показывает квоту триала под карточкой
  реального тарифа («0 из 150» под Бизнес); даунгрейд Apple применяет на продлении,
  приложение не пишет «с <дата> тариф сменится»; строка резерва докупки есть только с билда 3.
