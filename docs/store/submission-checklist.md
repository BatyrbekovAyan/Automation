...
- [x] **Скриншоты ГОТОВЫ (2026-09-03)** — витринные кадры с рамкой телефона и подписями,
  ...
  Не выкладывать: профиль, пейволл (цены могут разойтись с SKU; для ревью подписок
  ASC просит отдельный скриншот покупки — снять его с пейволла, когда SKU утверждены),
  промпт. Android phone — когда дойдёт Android-трек (композер отдаст 1080×2340 сменой
  `CANVAS`). iPad-скриншоты НЕ нужны: сборка iPhone-only (2026-09-01,
  `Tools/Store Compliance/Apply iOS Store Settings`). Демо-данные вымышленные; номер
  клиента `+7 705 318-24-90` заменить на подконтрольный владельцу перед подачей.
