...
- Ресабмит: билд 3 (архив Xcode → ASC → TestFlight), проверка в состоянии ревьюера
  (настоящий Apple ID выйти из «Контент и покупки», US sandbox-тестер в Настройки →
  Разработчик, свежая установка, канал НЕ подключать → Профиль → Подписка → Изменить
  тариф → большая кнопка открывает окно Apple → покупка), видео этой покупки → «Ответ на
  проверку приложения» с путём + видео → на странице версии выбрать билд 3 →
  «Update Review» → «Resubmit to App Review».
