...
- [ ] **Ревизия после opus-ревью (обязательные правки):** (1) PurchasesConfiguration.Builder — явные Set: StoreKitVersion=Default, EntitlementVerificationMode=Informational, ShouldShowInAppMessagesAutomatically=true (голый Build() молча пинит StoreKit1 и глушит store-биллинг-сообщения); (2) BillingService.Initialize в Manager.Start — в try/catch И ПОСЛЕ прогрева Secrets (первый Secrets.Data-рид на Android иначе идёт блокирующим LoadFromApkBlocking; необработанный throw из нового SDK брикует запуск за LoadingPanel); (3) `_configured`-флаг: Purchase/Restore в окне деферра Configure отвечают "not_initialized"-колбэком, не трогают несетапленный wrapper; (4) FinishConfigure: try/catch + лог ОБЕИХ ошибочных веток GetCustomerInfo (молчаливый сплит app_user_id недопустим), AppUserId ре-капчерится в Apply/CustomerInfoReceived; (5) tri-state: `BillingService.EntitlementsKnown` (false между keyed-Initialize и первым CustomerInfo/ошибкой) — EntitlementGate при !EntitlementsKnown && purchased==None даёт Trial-grace, НЕ None (платник с истёкшим локальным триалом не должен ловить пейволл на время резолва; enforcement всё равно серверный); (6) RevenueCatBackend.Purchase выводит product type из SKU: SkuTopUp → "inapp", иначе "subs" (иначе consumable уйдёт как subs — на Android упадёт). Minors туда же: ResetSeamsForTests развязывает PurchasedTierSource; комментарий ResolveApiKey; RestorePurchases логирует error-строку.
...
### Task 13b: кэш-паддинг статического префикса (решение владельца 2026-08-22: «паддинг»)

**Контекст (Task 13, эмпирика):** OpenAI кэширует префикс надёжно только от ~2200 номинальных токенов (16 вызовов на ~1300 — ноль хитов); боты с коротким профилем (мало описания/товаров, без доп. инструкций) работают без кэша ≈7,3 ₸/диалог вместо ≈4 ₸. Решение: гарантировать каждому боту статический systemMessage ≥ ~2300 токенов за счёт УНИВЕРСАЛЬНОГО блока правил, который сам кэшируется (cached input ×0.1 — паддинг почти бесплатен).

**Files:**
- Create: `Tools/n8n/prompts/_universal.md` — ~1200–1400 токенов RU: развёрнутые пояснения и примеры к УЖЕ существующим правилам `_core.md` (тон, структура ответа, хеджирование наличия/цен, эскалация «передам владельцу», RU/KZ зеркалирование, обработка «Каспи рассрочка», чего не делать). **Поведенчески нейтрально: никаких НОВЫХ ограничений, никаких противоречий `_core.md` и вертикальным промптам** — цель стабильные токены при нулевом дрейфе поведения.
- Modify: `Tools/n8n/inject-prompts.py` — композиция `_core + <vertical> + _universal` (маркер-обёрнуто, идемпотентно, `--check` ловит рассинхрон).
- Modify (через инжектор, НЕ руками): 4 оркестратора Create/Edit × WA/TG — канонические JSONы в `Tools/n8n/workflows/` (инвариант каноники: инжектор правит файлы напрямую, dev — через API/REST; никаких dev-значений в канонику) + dev live (publish!).

**Гейты:**
1. `python3 Tools/n8n/inject-prompts.py --check` зелёный; `verify-telegram-parity.py` зелёный.
2. Токен-проба: композиция для МИНИМАЛЬНОГО профиля (пустое описание, 0 товаров, пустые доп. инструкции) ≥ 2300 токенов по tiktoken/o200k (или по `usage.prompt_tokens` живого вызова).
3. Живая проба по харнессу Task 13 (sha256-байт-идентичные пары, интервал ~1-2с): минимальный профиль → `cached_tokens > 0` на втором вызове.
4. Sanity поведения: 3 коротких диалога на dev до/после (один и тот же бот-шаблон, те же реплики) — ответы по тону/структуре эквивалентны; любая смена поведения = дефект паддинга, не фича.
5. Staleness: существующие клоны получают паддинг только при пересоздании/редактировании — восстановить в отчёте.

- [ ] Commit: `feat(n8n): universal static prompt block — every bot's prefix clears the caching floor`
...
### Task 16: RC TRANSFER — переезд подписки на новый app_user_id и гашение зомби-личности

**Откуда:** device pass 2026-08-26 (блок «DEVICE PASS» в `.superpowers/sdd/progress.md`). При
переустановке RC/StoreKit2 САМ перенёс подписку на свежий анонимный id (`…819a2` → `…c2b9a`), и
вскрылись две дыры: **GAP-1** — купленный топ-ап остался на СТАРОМ id (новый видит 0), **GAP-2** —
старая личность становится зомби: её строка в `subscribers` навсегда `active` (событий по ней больше
не будет никогда), профили Wappi живут и жгут 23₽/день, клоны воркфлоу продолжают ОТВЕЧАТЬ, и ни одна
ветка свипа их не заберёт (`status='active'` исключён везде — сознательно). Событие TRANSFER на вебхук
в тот прогон не пришло вовсе (конфиг типов событий в дашборде RC — на владельце), но обработчик обязан
существовать в любом случае.

**Files:**
- Modify: `Tools/n8n/workflows/ZGYr6srzS3rSSXHp-RevenueCat_Events.json` (+ живой dev-воркфлоу)
- Modify: `Tools/n8n/probe-billing.py` (Part 6), `Tools/n8n/README.md` (строка таблицы)

**Что сделано:**
- `Map Event` распознаёт `type === 'TRANSFER'`. В пейлоаде RC НЕТ ни `app_user_id`, ни `product_id`,
  ни `entitlement_ids` — только массивы `transferred_from`/`transferred_to` (сверено с доками RC), так
  что через существующий гейт `If Has Payload?` это событие пройти физически не может: новый узел
  `If Is Transfer?` стоит ПЕРЕД ним и уводит такой айтем на отдельную ветку. Пустые массивы → no-op.
- `Transfer Subscriber` — ОДИН параметризованный statement (= одна транзакция, перенос и гашение не
  могут примениться наполовину): снимок плана/статуса/периода/SKU переезжает на новый id, а топ-ап
  СУММИРУЕТСЯ с тем, что у него уже было; в том же statement каждая строка-источник получает
  `status='expired'`, `topup_balance=0` и `current_period_end`, ЗАЖАТЫЙ до `now()`.
- Снимок переезжает только если источник СТРОГО свежее получателя: `RENEWAL` под новым id законно
  приходит раньше TRANSFER, а RC ретраит любой не-2xx — ни гонка, ни повторная доставка не имеют права
  понизить здоровую строку (ничья отдаётся получателю, что и делает повторный TRANSFER no-op'ом,
  включая топ-ап).
- **Никакой новой машинерии удаления не строится.** `status='expired'` гасит автоответы через
  существующий квота-гейт (`Count Dialog` не открывает НОВЫЙ диалог при `status in ('expired','grace')`),
  а клэмп периода отдаёт профили ветке A свипа `Profile Lifecycle Sweep`, чей `Candidates` смотрит
  ровно на `status in ('expired','grace') and current_period_end < now() - interval '3 days'`.
  Оплаченный период, оставшийся в будущем, продержал бы зомби-профили до месяца — проверено
  контрольной фикстурой (без клэмпа кандидатом не становится).
- Бонусом там же: `PRODUCT_CHANGE` несёт СТАРЫЙ sku в `product_id` и новый в `new_product_id` —
  маппер теперь предпочитает второй (живое наблюдение того же прогона: апгрейд записал
  `sub.start.month` поверх плана business).

- [x] Каноника правится ХИРУРГИЕЙ от закоммиченной базы, не экспортом dev; publish через
      `PUT` + `POST /activate`, `versionId == activeVersionId` проверен.
- [x] Проба `probe-billing.py --transfer`: 8 шагов на ЗНАЧЕНИЯХ (сумма топ-апа, клэмп, реплей,
      пустые массивы); дефолтный прогон (Part 1) дополнительно получил ассерт на `new_product_id`.
- [x] `verify-telegram-parity.py` — ALL PASSED.

#### Fix round (adversarial review, тот же день): СКЛЕЙКА ИДЕНТИЧНОСТЕЙ — вот что происходит на самом деле

**Главный факт, найденный ревью в сырых событиях этого инстанса: при переустановке на том же
устройстве RC НЕ ШЛЁТ `TRANSFER` — он СКЛЕИВАЕТ идентичности.** exec 3144 (до переустановки):
`app_user_id=OLD, aliases=[OLD]`. execs 3150/3151/3153 (после): `app_user_id=NEW,
original_app_user_id=OLD, aliases=[NEW, OLD]`, а `transferred_from/to` в них ВООБЩЕ НЕТ. Доки RC
прямо предписывают «search both the original_app_user_id and the aliases array». То есть
обработчик TRANSFER закрывал только МЕЖАККАУНТНЫЙ переезд, а GAP-1/GAP-2 из device pass —
переустановка — оставались открытыми.

- **B1. `Consolidate Aliases`** — новая ветка (`If Needs Consolidation?` → Postgres), которая
  на каждом событии сводит алиасы в текущий id. Четыре ограничения, каждое несущее:
  (1) **только перенос баланса и гашение, НИКАКОГО копирования снимка** — событие, едущее следом,
  уже несёт авторитетные план/статус/период, а этот путь срабатывает ВЕЧНО, на каждом событии;
  (2) **строго ДО `Upsert Subscriber`** — `topup_balance + $5` в апсерте не идемпотентен к ретраю,
  падение ПОСЛЕ него дало бы двойное начисление топ-апа самого события при передоставке;
  (3) **сумма добывается ПОД БЛОКИРОВКОЙ** (`for update`), а не снимочным `sum()` — инстанс
  доставляет события одного подписчика параллельно (3128/3129 перекрылись на 2.4с, 3150/3151 — в
  ту же секунду); (4) **гард на гашении** — устоявшийся набор алиасов не плодит версии строк.
  Нода отдаёт payload события через `select $1..$7` (Postgres-нода роняет входной json).
- **B2. NULL-период = МГНОВЕННОЕ удаление, а не grace.** `Candidates` читает
  `coalesce(current_period_end, now() - interval '99 days')`, поэтому погашенная строка с NULL
  становится кандидатом сразу же: топ-ап-only личность (её план/статус/период `NON_RENEWING_PURCHASE`
  не трогает) потеряла бы РЕАЛЬНЫЕ профили Wappi за ≤6 часов. Клэмп на ОБОИХ путях теперь
  `least(coalesce(period, now()), now())`.
- **B3. Срок ≠ признак жизни.** Истёкший источник с более поздним периодом штамповал бы
  `status='expired'` поверх живого `trialing` (у которого период NULL = -infinity и он всегда
  проигрывает) и заглушил бы свежую установку через `Count Dialog`. Добавлен liveness-терм:
  снимок отклоняется, если получатель `active|trialing`, а источник `expired|grace`. Деньги
  переезжают всё равно.
- **I-3.** Снимок раздаётся ВСЕМ id из `transferred_to` («App User ID(s) receiving»), деньги — только
  первому. **M-4.** У путей TRANSFER/склейки свои 500-ответы (`transfer_failed`/`consolidation_failed`).

- [x] Проба переписана: 5 кейсов TRANSFER (happy+реплей+пустые массивы, RENEWAL раньше TRANSFER,
      истёкший источник против живого получателя, NULL-период, мульти-id) + кейсы склейки
      (A1 склейка и пронос payload, A2 реплей, A3 4 параллельные доставки → начислено ровно раз).
- [x] `--branch-a-trace` — req-3 как воспроизводимый режим пробы: гоняет РОДНОЙ `Candidates`
      свипа против настоящих строк + контрольная строка без клэмпа (кандидатом не становится).
- [x] Негативный тест против v1 (6d417ef): 11 ассертов падают ровно по B1/B2/B3/I-3.

---

### Task 17a: серверные правки финального ревью — резерв топ-апа, гейт подсказок, перенос расхода

**Откуда:** блок «FINAL WHOLE-EPIC REVIEW — 2026-08-26» в `.superpowers/sdd/progress.md`
(C-1, I-3, I-4, минорки M-1/M-2/M-3) + решения владельца 2026-08-26 (spec §2 — топ-ап это
РЕЗЕРВ; §5.3 — подсказки бесплатны, но гейтуются подпиской + дневной анти-абьюз потолок).
Правится только `Tools/n8n/**`; клиентская половина — Task 17b (параллельно, без пересечения файлов).

- [x] **C-1 (оба бот-шаблона).** `Count Dialog`: CTE `quota` — только БАЗОВАЯ квота плана (слагаемое
      `+ topup_balance` удалено), а новый CTE `reserve` списывает ровно одну единицу резерва
      `update subscribers … set topup_balance = topup_balance - 1 … where topup_balance > 0`, и ТОЛЬКО
      для НОВОГО диалога (`not exists(existing)`) сверх исчерпанной квоты (`used >= q`) у статуса
      `active`/`trialing`. Блокировка строки, которую берёт этот UPDATE, и есть контроль
      конкурентности: READ COMMITTED перепроверяет `> 0` уже на свежей версии строки, поэтому две
      гонящиеся новые беседы дают «один разрешён / один отказан», а баланс не уходит в минус.
      `allowed` включает `exists(reserve)`: `on conflict do nothing` может проглотить НАШУ вставку,
      если гонщик успел записать тот же ключ, — конфликт ДОКАЗЫВАЕТ, что строка есть, иначе гонка
      списала бы единицу И отказала. `Get_Usage` не менялся: его `quota` и была базовой, а
      `topupBalance` теперь ЗНАЧИТ «остаток резерва».
- [x] **I-3 (`Suggest_Replies`).** Ветка «валидный запрос» из `If invalid?` теперь идёт в
      `Suggestion Gate` (Postgres, fail-open как `Count Dialog`) → `Gate Decision` (Code; собирает
      payload из `$('Prep')` — Postgres-нода роняет входной item, ловушка Task 9) → `If Gate Allows`:
      true — обычный путь `If skipRag?`, false — сразу `Build Response`. Отказ не доходит НИ до
      retrieval, НИ до LLM (доказано путём нод: 3813/3824/3833 останавливаются на Build Response).
      Один statement делает поиск + инкремент + вердикт по новой таблице `suggestion_counts`
      (миграция `Tools/n8n/sql/2026-08-26-suggestion-counts.sql`, PK `(app_user_id, d)`, день по
      Asia/Almaty, потолок 100/сутки — 101-й запрос первый отказной). Отказ: `expired`, id без
      строки в `subscribers` (id — единственный секрет этого неаутентифицированного вебхука) и
      пустой/отсутствующий `appUserId` (клиент шлёт его ВСЕГДА — Task 17b R3). Разрешено:
      `active`/`trialing`/`grace`, в том числе СВЕРХ КВОТЫ — панель и есть фолбэк. Конверт ответа
      прежний (`generation_failed`) + диагностический `reason`; новых состояний панели нет.
- [x] **I-4 (`RevenueCat_Events`).** `Consolidate Aliases` и `Transfer Subscriber` переносят строки
      `dialog_counts` ТЕКУЩЕГО месяца на живой id (`insert … select distinct … on conflict do
      nothing`): без этого переустановка обнуляла оплаченный счётчик. Существование строки И ЕСТЬ
      счёт, поэтому копия идемпотентна по построению, а settled-повтор пишет НОЛЬ новых версий строк
      (измерено по `xmin`). На пути TRANSFER расход едет вместе со СНИМКОМ (на все `transferred_to`),
      деньги — по-прежнему только первому.
- [x] **Минорки.** M-1: `Register Profile` гарантирует строку `subscribers` тем же statement'ом
      (CTE `insert … on conflict do nothing`) — закрыто окно «Ensure Subscriber упал ⇒ бот без учёта
      квоты и невидим для свипа». M-3: `Register Profile` отказывает `profile_id` `'-1'`/пустому
      (сентинел клиента против ПЕРВИЧНОГО КЛЮЧА). M-2: `Compute Slot Limit` даёт 0 слотов и на
      `grace` — `Count Dialog` там уже молчит, а слоты означали бы новые платные Wappi-профили.
- [x] **Гейт.** `verify-telegram-parity.py`: `check_dialog_metering` проверяет НОВУЮ форму SQL
      (базовая квота без топ-апа, гард `> 0`, область резерва, терм в `allowed`), новый
      `check_dialog_metering_shared` требует БАЙТОВОГО равенства метринга в обоих шаблонах (это и
      есть доказательство для WhatsApp: пиновать живьём можно только TG — он `availableInMCP`),
      `check_suggest_replies` — весь гейт (позиция до RAG/LLM, fail-open, `dialog_counts` не
      трогается, потолок, набор статусов, непрерывность payload, конверт ответа). Все ассерты
      читают SQL БЕЗ комментариев (`sql_code`) — иначе комментарий удовлетворяет ассерт про код.
      18 мутаций — 18 падений с правильным сообщением.
- [x] **Проба.** `probe-billing.py --reserve` (матрица из 8 сценариев + слив до нуля + настоящая
      гонка с `pg_sleep`, ШИПАЕМЫЙ SQL читается из канонического JSON, не перепечатывается) и
      `--suggestions` (11 ассертов по живому вебхуку). A1/A2 в `--transfer` дополнены проверкой
      `used`: расход переезжает и не растёт на реплее.

---

### Task 17b: клиентские правки финального ревью — effective-«Вместе», резерв в счётчике, личность панели

**Откуда:** блок «FINAL WHOLE-EPIC REVIEW — 2026-08-26» в `.superpowers/sdd/progress.md` (I-1,
клиентские половины C-1 и I-3, минорки M-4/M-6/M-10) + решения владельца 2026-08-26 (spec §2,
примечание про add-on, и §5.3): топ-ап — РЕЗЕРВ (тратится по одному диалогу только сверх
исчерпанной месячной квоты, не сгорает), подсказки «Вместе» бесплатны, но гейтуются подпиской.
Серверная половина — Task 17a.

**Files:**
- Add: `Assets/Scripts/Billing/QuotaFallbackPolicy.cs` + `Assets/Tests/Editor/Billing/QuotaFallbackPolicyTests.cs`
- Modify: `QuotaMath.cs`, `BotsPageRows.cs`, `SubscriptionPageRows.cs`, `PaywallController.cs`,
  `Chat/SuggestionsController.cs`, `Chat/N8nSuggestionsProvider.cs`, `Chat/SuggestRepliesDtos.cs`,
  `Main/BotsPageBilling.cs`, `Main/ProfileSubPages.Subscription.cs`, `Main/BotSettings.Auth.cs`
  (+ 5 тест-файлов). Сцена/префабы НЕ трогались.

**Что сделано:**
- **I-1 — over-quota показывает «Вместе», а не тишину.** Новый чистый сим `QuotaFallbackPolicy`:
  фолбэк включается только при ИЗВЕСТНОМ снимке (`UsageStore.Current != null && success` —
  «ещё не читали» никогда не читается как «ноль»), обслуживаемом статусе (`active|trialing|grace`;
  `expired`/пустой/незнакомый — нет, иначе панель поднимется под отказ гейта 17a и владелец получит
  карточку ошибки вместо тишины), `quota > 0`, `used >= quota` и пустом резерве. Проводка — ОДНО
  место, где режим чата РАЗРЕШАЕТСЯ (`RestoreForActiveChat`): `_semiAutoOn = EffectiveSemiAuto(
  SemiAutoStore.IsOn(...))`. Ни одной записи: ни `SemiAutoStore`, ни `ReplyModeToggleBinder`, ни
  серверного `Set_Reply_Mode` (тот пуш висит на явном три-стейте `TryGetOverride`, до которого
  оверрайд не дотягивается) — выбранный владельцем режим обязан вернуться нетронутым, как только
  квота сбросится или придёт топ-ап. `HandleToggle` намеренно НЕ переопределяется: тумблер,
  который нельзя выключить, — мёртвый контрол, а не фолбэк. Машина состояний слота не тронута.
- **C-1 (клиент) — резерв в счётчике.** `QuotaMath.State` получил четвёртое состояние `Reserve`
  (квота исчерпана, резерв ещё платит) и больше не считает потолок как `quota + topup`: под
  резервной семантикой `used` растёт, а `topup_balance` падает, поэтому старая формула закрывала
  разрыв ВДВОЕ быстрее и объявляла стену на половине резерва (1000+500 → «Over» на used 1250).
  `Remaining` = остаток квоты + ВЕСЬ резерв. Знаменатель счётчика — всегда БАЗОВАЯ квота
  (`UsageLine`, `FillFraction`; у последней параметр резерва убран — он не удлиняет дорожку),
  числитель не подрезается: «1 240 из 1 000» — честный счёт месяца. Полоса и подпись в состоянии
  `Reserve` — янтарные (не красные: стены нет), новая строка-подсказка
  «Квота исчерпана — из резерва осталось 320 диалогов» (глагол через `RuPlural`, существительное
  и NBSP через `PaywallCopy.Dialogs`). `OverHint` оставлен дословно — с I-1 он наконец правдив.
- **I-3 (клиент) — личность панели.** Пейлоад `SuggestReplies` получил аддитивный ключ v1.4
  `appUserId` = `BillingIdentity.AppUserId` (тот же источник, что у форм Create/Edit и GetUsage).
  Шлётся ВСЕГДА и передаётся дословно — без клэмпа, без дефолта, никогда не выдумывается: гейт 17a
  ТРЕБУЕТ поле, а отказ приезжает в существующем конверте ошибки, который панель уже рисует
  (новых состояний UI нет). Тест v1-идентичности расширен: снятие
  `channel+botTgId+businessKnowledge+now+pickStats+appUserId` по-прежнему даёт замороженный
  12-ключевой v1.
- **Ride-alongs.** M-6: успешная покупка тарифа (и восстановление) в `PaywallController` дёргает
  `UsageClient.FetchRoutine()` — на `Manager.Instance`, а НЕ на себе: `Close()` строкой ниже гасит
  GameObject, а Unity убивает корутину вместе с хостом. M-4: `PlanCatalog.SkuTopUp` закреплён
  drift-тестом (литерал дублируется в `Map Event` воркфлоу RC_Events). M-10: восемь литералов
  `"-1"` в `BotSettings.Auth.cs` заменены на `Bot.UnauthedProfileSentinel`.

- [x] Полный EditMode-прогон в открытом редакторе: **2328/2328** (было 2305 + 23 новых теста),
      свежесть подтверждена штампом `Assembly-CSharp-Editor.dll` в сводке прогона.

**Осталось (записано, не сделано):** на «Подписке» нет строки-подсказки (её носитель — только
полоса «Боты»), поэтому остаток резерва там не назван — это потребовало бы правки сцены; фолбэк
I-1 вычисляется в момент ОТКРЫТИЯ чата (исчерпание квоты, замеченное при уже открытом чате,
всплывёт на следующем открытии); дозапрос usage после покупки может обогнать вебхук RevenueCat —
та же гонка, что и у давно живущего топ-апа.
