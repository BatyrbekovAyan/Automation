...
- [ ] Step 1: тест на `CountChannels` + «gate возвращает false и зовёт `RequestPaywall`» (событие ловится в тесте лямбдой). Step 2: FAIL. Step 3: реализация + врезки: в Manager перед открытием мастера — `if (!EntitlementGate.CanCreateBot(count)) { EntitlementGate.RequestPaywall(PaywallTrigger.BotLimit); return; }`; в BotSettings.Auth перед стартом QR/кода — то же с `ChannelLimit`. Step 4: PASS + ручная проверка в Editor (лимит 0 при None — мастер не открывается). Step 5: commit `feat(billing): entitlement gate wired into bot/channel creation`.
